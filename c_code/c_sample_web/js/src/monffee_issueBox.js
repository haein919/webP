...
(function($){
  ...
  // ------------------------------------------------------------
  // 1. 인디케이터 list를 클릭 시 각 내용으로 위치 이동
  var permission = true;
  var itIndex = 0;

  // 함수수행기능 별도로 처리
  var SlideFn = function(itIndex){
    issueUl.stop().animate({ 'margin-left':itIndex * -100 +'%' }, timed, function(){
  ...
  issueIndiLi.on ('click', ['a'], function(e){
    ...
    if(permission){
      ...
      itIndex = $(this).index(); //20행에 있는 var itIndex와 같음
      SlideFn(itIndex);
    }
  });
// ---------------------------------------------------------------------------------------
...
var SlideGoFn = function(){
  mySlideGo = setInterval(function(){
    ...
    itIndex += 1;
    if(itIndex > issueLiLen-1){ itIndex = 0; }
      SlideFn(itIndex);
  }, timed * 3);
};
